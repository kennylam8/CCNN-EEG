...
validate = 0
test = 30
n_features, train_input, train_target, test_input, test_target = get_data(train, validate, test)  # refer to build_data.py the
...
class CasPer(torch.nn.Module):
    ...
    def add_neuron(self):
        self.new_hidden = torch.nn.Linear(self.iteration - 1 + input_neurons, 1)
