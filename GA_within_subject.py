...
def ccs3 (cutoff, k , cms,max_iter,constant_epoch):
    ...
    for i in range(5):
        ...
        test_input = test_data.iloc[:, :n_features]
        test_target = test_data.iloc[:, n_features]
        model = main_casper(n_features, train_input, train_target, test_input, test_target, cutoff, k, cms, max_iter,constant_epoch, "within-subject", train_data) #TODO CUTOFF HERE
